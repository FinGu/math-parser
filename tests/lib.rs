...
pub mod tests {
    ...
    fn postfix_to_result_valid1() {
        let t = postfix_to_result::new("8 21 * 89 14 / + 2 ^").parse(&mut None).unwrap();
    ...
    fn postfix_to_result_valid2() {
        let t = postfix_to_result::new("50 45 m *").parse(&mut None).unwrap();
    ...
    fn postfix_to_result_failed1() {
        postfix_to_result::new("50 45 + -").parse(&mut None).unwrap();
    }

    #[test]
    #[should_panic]
    fn postfix_to_result_failed2() {
        postfix_to_result::new("+").parse(&mut None).unwrap();
    }
}
